...
class Factor(object):
    ...
    def _set_states(self, states):
        ...
        for RV, values in self.states.items():
            self.name_to_number[RV] = {
                name: nr for nr, name in enumerate(self.states[RV])
            }
            self.number_to_name[RV] = {
                nr: name for nr, name in enumerate(self.states[RV])
            }
    ...
    def align_index(self, other, inplace=False):
        ...
        if remaining_vars:
            ...
            indices = list(
                map(_combine_indices, product(shared_indices, remaining_indices))
            )
    ...
    def as_series(self):
        ...
        idx = pd.MultiIndex.from_product(self.states.values(), names=self.states.keys())
    ...
    def states_to_index(states):
        ...
        idx = pd.MultiIndex.from_product(states.values(), names=states.keys())
