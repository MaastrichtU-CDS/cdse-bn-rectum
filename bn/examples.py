...
def get_student_CPTs():
    ...
    P["I"] = CPT([0.7, 0.3], states=subset(states, ["I"]), description="Intelligence")
    ...
    P["D"] = CPT([0.6, 0.4], states=subset(states, ["D"]), description="Difficulty")

    P["G"] = CPT(
        [0.30, 0.40, 0.30, 0.05, 0.25, 0.70, 0.90, 0.08, 0.02, 0.50, 0.30, 0.20],
        states=subset(states, ["I", "D", "G"]),
        description="Grade",
    )

    P["L"] = CPT(
        [0.10, 0.90, 0.40, 0.60, 0.99, 0.01],
...
def get_sprinkler_factors():
    ...
    # P(A)
    fA = Factor([0.6, 0.4], subset(states, ["A"]))

    # P(B|A)
    fB_A = Factor([0.2, 0.8, 0.75, 0.25], subset(states, ["A", "B"]))

    # P(C|A)
    fC_A = Factor([0.8, 0.2, 0.1, 0.9], subset(states, ["A", "C"]))

    # Define a factor that holds the *conditional* distribution P(D|BC)
    fD_BC = Factor(
        [0.95, 0.05, 0.9, 0.1, 0.8, 0.2, 0.0, 1.0], subset(states, ["B", "C", "D"])
    )

    # P(E|C)
    fE_C = Factor([0.7, 0.3, 0.0, 1.0], subset(states, ["C", "E"]))
...
def get_example7_factors():
    ...
    # P(B|A)
    fB_A = Factor([0.9, 0.1, 0.2, 0.8], subset(states, ["A", "B"]))

    # P(C|A)
    fC_B = Factor([0.3, 0.7, 0.5, 0.5], subset(states, ["B", "C"]))
...
def get_example7_network():
    ...
    bn = BayesianNetwork("class", [nA, nB, nC], [("A", "B"), ("B", "C")])
...
def get_example17_2_network():
    ...
    bn = BayesianNetwork("Health", [nH, nS, nE], [("H", "S"), ("H", "E")])
...
def get_example17_3_factors():
    ...
    # P(A)
    fA = Factor([0.2, 0.8], subset(states, ["A"]))

    # P(B|A)
    fB_A = Factor([0.75, 0.25, 0.10, 0.90], subset(states, ["A", "B"]))

    # P(C|A)
    fC_A = Factor([0.5, 0.5, 0.25, 0.75], subset(states, ["A", "C"]))

    # P(D|B)
    fD_B = Factor([0.2, 0.8, 0.7, 0.3], subset(states, ["B", "D"]))
...
def get_example17_3_network():
    ...
    positions = {"A": [184, 23], "B": [58, 117], "C": [288, 117], "D": [58, 222]}
    ...
    bn = BayesianNetwork(
        "Example 17.3", [nA, nB, nC, nD], [("A", "B"), ("A", "C"), ("B", "D")]
    )
