...
class BayesianNetwork(ProbabilisticModel):
    ...
    def complete_case(self, case, include_weights=True):
        ...
        try:
            jpt = self.compute_posterior(qd=missing, qv={}, ed=[], ev=evidence)
    ...
    def compute_posterior(self, qd, qv, ed, ev, use_VE=False):
        ...
        log.debug(
            f"Found a node in the JT that contains {required_RVs}: {node.cluster}"
        )
        self.junction_tree.reset_evidence()
        self.junction_tree.set_evidence_hard(**ev)
    ...
    def as_bag(self):
        return Bag(name=self.name, factors=[n.cpt for n in self.nodes.values()])
...
class DiscreteNetworkNode(Node):
    ...
    def __init__(
        self, RV, name=None, states=None, description="", cpt=None, position=None
    ):
    ...
    def from_dict(cls, d):
        ...
        node = DiscreteNetworkNode(
            RV=d["RV"], name=d["name"], states=d["states"], description=d["description"]
        )
